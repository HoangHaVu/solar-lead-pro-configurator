  ...
  return (
    <div className="w-full max-w-lg mx-auto rounded-2xl overflow-hidden shadow-lg border border-slate-200 bg-white text-sm select-none">
      ...
      <div className="px-6 pt-5 pb-3 border-b border-slate-100">
        <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-1">Angebot für</p>
        <p className="font-bold text-slate-800">{MOCK.name}</p>
        <p className="text-slate-500 text-xs">{MOCK.zip}</p>
      </div>

      {/* System-Übersicht */}
      <div className="px-6 py-4 border-b border-slate-100">
        <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-3">Ihre Photovoltaikanlage</p>
          ...
          <div className="mt-3 space-y-1">
            {panelHersteller && (
              <p className="text-xs text-slate-500">
                <span className="font-semibold text-slate-700">Module:</span> {panelHersteller.split(',')[0].trim()}
              </p>
            )}
            {wechselrichterHersteller && (
              <p className="text-xs text-slate-500">
                <span className="font-semibold text-slate-700">Wechselrichter:</span> {wechselrichterHersteller.split(',')[0].trim()}
              </p>
            )}
          </div>
        )}
      ...
      {/* Zahlungsplan */}
      <div className="px-6 py-4 border-b border-slate-100">
        <p className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-3">Zahlungsplan</p>
        <div className="space-y-2">
          {[
            ...
            <div key={label} className="flex items-center justify-between">
              ...
              <p className="font-black text-slate-800 text-sm">{fmt(amount)} €</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {(iban || zahlungsziel) && (
        <div className="px-6 py-3 border-b border-slate-100 bg-slate-50">
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-500">
            {iban && <span><span className="font-semibold text-slate-700">IBAN:</span> {iban}</span>}
            {zahlungsziel && <span><span className="font-semibold text-slate-700">Zahlungsziel:</span> {zahlungsziel} Tage</span>}
          </div>
        </div>
      )}
