...
export type UserRole = 'customer' | 'installer' | 'owner';
...
export async function fetchProfile(userId: string): Promise<Profile> {
  const { data, error } = await supabase
    .from('profiles')
    .select('id, role, full_name, phone, zip, is_verified')
    .eq('id', userId)
    .single();
...
export async function signUpCustomer(
  ...
  // Rolle + Name als user_metadata → Datenbank-Trigger legt Profil automatisch an
