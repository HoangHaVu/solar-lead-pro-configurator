// Supabase Edge Function — notify-beta
...
Deno.serve(async (req) => {
  ...
  if (!resendKey) {
    // Kein Resend-Key konfiguriert — trotzdem 200 zurückgeben (Daten sind in DB)
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #0D2137; color: white; padding: 24px; border-radius: 12px 12px 0 0;">
        <h1 style="margin: 0; font-size: 20px;">☀️ Neue Beta-Anfrage</h1>
      </div>
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px; padding: 24px;">
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            ...
            <td style="padding: 8px 0; font-weight: bold; color: #0D2137;">${data.company_name}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; color: #64748b; font-size: 13px;">Ansprechpartner</td>
            <td style="padding: 8px 0; font-weight: bold; color: #0D2137;">${data.contact_name}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding: 8px 0; font-weight: bold; color: #0D2137; font-size: 16px;">
              <a href="tel:${data.phone}" style="color: #0D2137;">${data.phone}</a>
            </td>
          </tr>` : ''}
          ${data.zip ? `
          <tr>
            <td style="padding: 8px 0; color: #64748b; font-size: 13px;">PLZ / Region</td>
            <td style="padding: 8px 0; color: #0D2137;">${data.zip}</td>
          </tr>` : ''}
          ${data.message ? `
          <tr>
            <td style="padding: 8px 0; color: #64748b; font-size: 13px; vertical-align: top;">Nachricht</td>
            <td style="padding: 8px 0; color: #0D2137;">${data.message}</td>
          </tr>` : ''}
        </table>
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'SolarKonfigurator Beta <noreply@vu-studio.de>',
      to: ADMIN_EMAIL,
      subject: `☀️ Beta-Anfrage: ${data.company_name} (${data.phone ?? data.email})`,
      html,
    }),
  });
